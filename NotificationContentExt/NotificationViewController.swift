...
class NotificationViewController: UIViewController, UNNotificationContentExtension {
    ...
    func didReceive(_ notification: UNNotification) {
        ...
        let imageData = NSData(contentsOf: imgAttachment.url)
        let image = UIImage(data: imageData! as Data)

        let buttonNormalStateAtt = notification.request.content.attachments[1]
        let buttonNormalStateImgData = NSData(contentsOf: buttonNormalStateAtt.url)
        ...
        let buttonHighlightStateImgData = NSData(contentsOf: buttonHighlightStateAtt.url)
